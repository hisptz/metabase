...
export class MetadataEffects {
  ...
  setCurrentMetadata$ = this.actions$.ofType(fromActions.MetadataActionTypes.SET_CURRENT_METADATA).
    withLatestFrom(this.store).
    pipe(tap(([action, state]: [fromActions.SetCurrentMetadataAction, fromReducer.State]) => {
      if (!state.metadataObject || (state.metadataObject && !state.metadataObject.entities[action.metadataId])) {
        this.store.dispatch(new fromActions.LoadMetadataAction({
          id: action.metadataId,
          url: action.metadataUrl,
          loading: true,
          loaded: false,
          importing: false,
          imported: false,
